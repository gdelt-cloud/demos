...
#
# Each entry: (accent_color_hex, inline_svg_path_d)
...
def _prepare_timeline(
    ...
    """Position events on a horizontal date axis, with 4 alternating lanes.

    Mirrors `nextjs_/components/demos/event-timeline.tsx`. Each row carries
    its pct-from-left, lane offset (-1.5..+1.5), dot radius, icon size,
    color, and SVG icon path.
    """
...
def _scale_entities(entities: list[dict[str, Any]]) -> dict[str, int]:
    """Max value per metric for relative bar sizing in the entity cards."""
    return {
...
def _sparkline_path(points: list[dict[str, Any]], width: int, height: int) -> str:
    """Smooth-ish SVG path (Q-curve smoothing) for a list of {date,value} points."""
    if len(points) < 2:
        return ""
...
def _macro_sparkline(label: str, unit: str, points: list[dict[str, Any]]) -> dict[str, Any] | None:
    """Build everything the template needs to render a single sparkline."""
    if len(points) < 2:
        return None
...
def render_dashboard(dataset: Dataset, output_dir: Path) -> Path:
    ...
    # Static macro snapshot for the standalone demo. In production the hosted
    # Next.js page pulls a live series from the GDELT Cloud MCP macro_finance
    # proxy; here we ship a hardcoded series so the demo is self-contained.
    brent_sparkline = _macro_sparkline(
        label="Brent Crude Oil",
        unit="USD/bbl",
        points=[
            {"date": "2026-04-21", "value": 84.6},
            {"date": "2026-04-28", "value": 86.1},
            {"date": "2026-05-05", "value": 88.4},
            {"date": "2026-05-12", "value": 87.9},
            {"date": "2026-05-19", "value": 89.2},
        ],
    )
    ...
    html = template.render(
        ...
        macro_series=[brent_sparkline] if brent_sparkline else [],
