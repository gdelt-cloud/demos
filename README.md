...
Three customer-shaped workflow dashboards built only on the GDELT Cloud public REST API. Each demo is a self-contained Python project that pulls structured events, story clusters, named entities, and (where relevant) energy infrastructure exposure for a specific operating picture, and renders a single static HTML report you can open in a browser or print to PDF.
...
**Hosted versions** (live, refreshed daily):
...
Each demo ships with a `SKILL.md` you can hand to Claude Code, Cursor, or Copilot CLI:

> "Use [`red-sea-watchboard/SKILL.md`](./red-sea-watchboard/SKILL.md) to scaffold a Suez Canal disruption watchboard for me."

The SKILL files describe a generic pattern (maritime threat / war-risk underwriting / commodity supply-chain) with a menu of regions and entity-anchors, so the agent can adapt the demo to your route, your commodity, your persona.
